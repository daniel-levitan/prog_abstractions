/**
 * File: simpleTextEditor.cpp
 * --------------------------
 * This file implements a simple command-driven text editor, which is used
 * to test the EditorBuffer class.
 */

#include <iostream>

#include "buffer.h"
#include "../../../libraries/simpio.h"
#include "myio.h"
#include <string>

using namespace std;

/* Function Prototypes */

void executeCommand(EditorBuffer & buffer, string line);
void displayBuffer(EditorBuffer & buffer);
void getParagraph(EditorBuffer & buffer);
void printHelpText();


int main() {
    EditorBuffer buffer;
    int i = 0;
    while (true) {
        // string prompt = to_string(i) + " *";
        // string cmd = getLine(prompt);
        string cmd = getLine("*");
        if (cmd != "") executeCommand(buffer, cmd);
        i++;
    }

    return 0;
}


/**
 * Function: executeCommand 
 * Usage: executeCommand(buffer, line);
 * ------------------------------------
 * Executes the command specified by line on the editor buffer.
 */
void executeCommand(EditorBuffer & buffer, string line) {
    switch(toupper(line[0])) {
        case 'I': for (int i = 1; i < line.length(); i++) { 
                      buffer.insertCharacter(line[i]);
                  }
                  break;
        case 'A': getParagraph(buffer); displayBuffer(buffer); break;          
        case 'D': buffer.deleteCharacter(); break;
        case 'F': buffer.moveCursorForward(); break;
        case 'B': buffer.moveCursorBackward(); break;
        case 'J': buffer.moveCursorToStart(); break;
        case 'E': buffer.moveCursorToEnd(); break;
        case 'T': displayBuffer(buffer); break;
        case 'H': printHelpText(); break;
        case 'Q': exit(0);

        /* delete after */
        case 'P': buffer.showPointers(); break;
        case 'S': buffer.showStacks(); break;
        case 'X': buffer.showArray(); break;

        default: cout << "Illegal command" << endl; break;
    }
}

/**
 * Function: getParagraph
 * Usage: getParagraph(EditorBuffer & buffer);
 * -------------------------------------------
 * 
 */
void getParagraph(EditorBuffer & buffer) {
    string paragraph = getMyLine();
    // string paragraph = "123";
    // cout << paragraph << endl;
    for (int i = 0; i < paragraph.length(); i++) { 
        // if (paragraph[i] != '\n')
            buffer.insertCharacter(paragraph[i]);
        // else
            // buffer.insertCharacter(' ');
    }
}

/**
 * Function: displayBuffer
 * Usage: displayBuffer(buffer);
 * -----------------------------
 * Displays the state of the buffer including position of the cursor.
 */
// void displayBuffer(EditorBuffer & buffer) {
//     // cout << buffer.getText() << endl;
//     string str = buffer.getText();
//     for (int i = 0; i < str.length(); i++) 
//         cout << " " << str[i];
//     cout << endl;

       /** 
        * Essa linha se perde quando tem \n, o calculo eh feito para 
        * posicionar o cursor em relacao ao texto, mas quando tem duas
        * linhas a conta nao eh trivial
        */
//     cout << string(2 * buffer.getCursor(), ' ') << "^" << endl;
// }   

/* Versao texto inteiro */
void displayBuffer(EditorBuffer & buffer) {
    cout << buffer.getText() << endl;
}

/**
 * Function: printHelpText
 * Usage: printHelpText();
 * -----------------------
 * Displays a message showing the legal commands.
 */

void printHelpText() {
    cout << "Editor commands:" << endl;
    cout << "  Iabc    Inserts the characters abc at the cursor position" << endl;
    cout << "  F       Moves the cursor forward one character" << endl;
    cout << "  B       Moves the cursor backward one character" << endl;
    cout << "  D       Deletes the character after the cursor" << endl;
    cout << "  J       Jumps to the beggining of the buffer" << endl;
    cout << "  E       Jumps to the end of the buffer" << endl;
    cout << "  H       Prints this message" << endl;
    cout << "  Q       Exits from the editor program" << endl;
}
